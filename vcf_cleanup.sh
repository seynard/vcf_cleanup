...
#Adjust filters 
############################################################################################
if [ ${limit_FS} == -999 ]; then limit_FS=$(cat ${DIROUT}/FS_value.txt); fi
if [ ${limit_FS} == 0 ]; then limit_FS=61; fi
if [ ${limit_SOR} == -999 ]; then limit_SOR=$(cat ${DIROUT}/SOR_value.txt); fi
if [ ${limit_SOR} == 0 ]; then limit_SOR=4; fi
if [ ${limit_MQ} == -999 ]; then limit_MQ=$(cat ${DIROUT}/MQ_value.txt); fi
if [ ${limit_MQRankSum} == -999 ]; then limit_MQRankSum=-12.5; fi
if [ ${limit_ReadPosRankSum} == -999 ]; then limit_ReadPosRankSum=-8; fi
if [ ${limit_QUAL} == -999 ]; then limit_QUAL=$(cat ${DIROUT}/QUAL_value.txt); fi
if [ ${limit_QD} == -999 ]; then limit_QD=$(cat ${DIROUT}/QD_value.txt); fi
if [ ${limit_GQ} == -999 ]; then limit_GQ=$(cat ${DIROUT}/GQ_value.txt); fi
if [ ${limit_miss} == -999 ]; then limit_miss=$(cat ${DIROUT}/miss_value.txt); fi
if [ ${limit_miss} == 0 ]; then limit_miss=0.05; fi
if [ ${limit_het} == -999 ]; then limit_het=$(cat ${DIROUT}/het_value.txt); fi
if [ ${limit_het} == 0 ]; then limit_het=0.01; fi
if [ ${limit_GQfiltered} == -999 ]; then limit_GQfiltered=$(cat ${DIROUT}/GQfiltered_value.txt); fi
if [ ${limit_GQfiltered} == 0 ]; then limit_GQfiltered=0.2; fi
echo 'FS' ${limit_FS}
echo 'SOR' ${limit_SOR}
echo 'MQ' ${limit_MQ}
echo 'MQRankSum' ${limit_MQRankSum}
echo 'ReadPosRankSum' ${limit_ReadPosRankSum}
echo 'QUAL' ${limit_QUAL}
echo 'QD' ${limit_QD}
echo 'GQ' ${limit_GQ}
echo 'miss' ${limit_miss}
echo 'het' ${limit_het}
echo 'GQfiltered' ${limit_GQfiltered}
############################################################################################
#Run filters
############################################################################################
...
if [ "${type}" == "haploid" ]	
	then
	bcftools +missing2ref ${vcf_new} -- -m > ${DIROUT}/tmp.vcf
	bcftools +setGT ${DIROUT}/tmp.vcf -- -t q -i 'GT="het"' -n "./." > ${DIROUT}/tmp2.vcf && mv ${DIROUT}/tmp2.vcf ${DIROUT}/tmp.vcf	
	bcftools view -i 'F_MISSING<'${limit_het}'' ${DIROUT}/tmp.vcf | bcftools query -f '%CHROM\t%POS\n' > ${DIROUT}/heterozygous_keep.txt

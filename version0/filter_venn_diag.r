...
#R
#dir='/work/project/dynagen/seynard/NewVcfCleanUp/results'library(VennDiagram)
library(data.table)
...
#info<-fread('info.txt',header=T,data.table=F)
#if('miss'%in%filter_info$filt){miss<-fread('missing.txt',header=T,data.table=F)
#	info$miss<-miss$Miss}
#if('het'%in%filter_info$filt){het<-fread('heterozygous.txt',header=T,data.table=F)
#	info$het<-het$Het}
#if('GQ'%in%filter_info$filt & 'GQfiltered'%in%filter_info$filt){gq<-list.files(path='.',pattern='GQfiltered_below')
#	gq<-fread(gq[1],header=T,data.table=F)
#	info$GQfiltered<-gq[,3]}
#for(i in 5:ncol(info)){info[,i][info[,i]=='.']<-0; info[,i]<-as.numeric(info[,i])}	
...
run_filter<-function(filter_list,filter_name,filter_value,threshold_direction,infile,previous_markers_name,previous_markers){
	...
	for(i in 1:length(filter_list)){
		if(filter_list[i]=='allele'){
			f_list[[i]]<-paste0(infile$CHROM[nchar(infile$ALT<=3) & grepl('\\*',infile$ALT)==F],'_',infile$POS[nchar(infile$ALT<=3) & grepl('\\*',infile$ALT)==F])
	...
	venn.diagram(
		...
		filename=paste0(dir,'/venn',paste0(name_filter,collapse='_'),'.pdf'),
		output=TRUE,
		cex=0.5,
		lwd=2,
		col=myCol,
